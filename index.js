...
ws.on('connection', function connection(ws, request, client) {
  ...
  ws.on('message', function incoming(message) {
    ...
    if(message.length === 4)
    {
      ...
      {
        console.log("wsid:",wsid, clientWSID);
        serverID = wsid;
        serverWS = ws;
          ...
          switch(message[1])
          {
              ...
              case 0:
                for (let i = 0; i < clients.size; i++) {
                  ...
                  if(clientWSID !== serverID)
                  {
                    console.log("compare id", clientWSID, serverID);
              ...
              case 2:
                for (let i = 0; i < clients.size; i++) {
                  ...
                  const idMsg = wsid.split('-');
                  // console.log("compare id:", clientWSID, wsid, idMsg[0]);
                  if(clientWSID !== wsid && clientWSID.includes(idMsg[0]))
                  // if(clientWSID !== wsid)
                  {
